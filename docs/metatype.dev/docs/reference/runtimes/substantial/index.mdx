...
The Substantial runtime enables the execution of durable workflows.

Why use it?

- **Long-Running processes**: Durable tasks that need to run over extended periods (days or weeks), handling **retries** and **restarts** seamlessly.
- **Fault-tolerant execution**: Ensure reliable execution of tasks, even in the face of server failures, by maintaining a durable state of the latest run.
- **Task orchestration**: Coordinate complex sequences of workflows (like microservices interactions).
...
def substantial_example(g: Graph):
    ...
    file = (
        ...
        .import_(["sendEmail", "sendEmailForAll"])
        .build()
    )
...
typegraph(
  ...
  (g) => {
    ...
    const file = WorkflowFile.deno("my_workflow.ts", [])
      .import(["sendEmail", "sendEmailForAll"])
      .build();
