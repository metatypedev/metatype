...
The simpile primitive typegraph types translate directly to their [GraphQL equivalent](https://graphql.org/learn/schema/#scalar-types).
...
Currently, all struct variants must have inline fragments present even the the user is not intersted in them.
