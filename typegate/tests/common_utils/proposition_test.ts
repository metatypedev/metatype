...
Deno.test("closest word proposition", async (t) => {
  await t.step("basic edgecases", () => {
    ...
    assertEquals(closestWord("", ["f", "first"], false, 1), "f");
    assertEquals(closestWord("", ["f", "first", ""], false, 1), "");
    // loosely equal
    assertEquals(closestWord("any", ["aNy", "ANY", "any"], true, 0), "aNy");
    // strictly equal
    assertEquals(closestWord("any", ["aNy", "ANY", "any"], false, 0), "any");
  });

  await t.step("default call", () => {
    // Note: ignoreCase: true by default
    assertEquals(closestWord("users", ["us", "user", "getUsers"]), "user");
    assertEquals(
