...
impl Pty {
    ...
    #[track_caller]
    fn read_until_condition(&mut self, mut condition: impl FnMut(&mut Self) -> bool) {
        let timeout_time = Instant::now().checked_add(Duration::from_secs(15)).unwrap();
...
mod windows {
    ...
    impl std::io::Write for WinPseudoConsole {
        ...
        fn flush(&mut self) -> std::io::Result<()> {
            // SAFETY: winapi call
            handle_err!(unsafe { FlushFileBuffers(self.stdin_write_handle.as_raw_handle()) });
            Ok(())
        }
    }
