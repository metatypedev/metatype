...
export class Auth {
  ...
  private static stdOauth2(provider: string, scopes: string, profiler: StdOauth2Profiler): RawAuth {
    switch (profiler.profiler) {
      case "none":
        return new RawAuth(wit_utils.oauth2WithoutProfiler(provider, scopes));
      case "extended":
        return new RawAuth(wit_utils.oauth2WithExtendedProfiler(provider, scopes, JSON.stringify(profiler.extension)));
      case "custom":
        return new RawAuth(wit_utils.oauth2WithCustomProfiler(provider, scopes, profiler.id));
      default:
        return new RawAuth(wit_utils.oauth2(provider, scopes));
    }
  }

  static oauth2Digitalocean(scopes: string, profiler?: StdOauth2Profiler): RawAuth {
    return Auth.stdOauth2("digitalocean", scopes, profiler ?? defaultProfiler());
  }
  ...
  static oauth2Instagram(scopes: string, profiler?: StdOauth2Profiler): RawAuth {
  ...
  static oauth2Microsoft(scopes: string, profiler?: StdOauth2Profiler): RawAuth {
  ...
  static oauth2Stackexchange(scopes: string, profiler?: StdOauth2Profiler): RawAuth {
    return Auth.stdOauth2("stackexchange", scopes, profiler ?? defaultProfiler());
  }
