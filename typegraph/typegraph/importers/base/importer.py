...
class Importer:
    ...
    def codegen(self, cg: Codegen, tg_alias="g"):
        ...
        if len(self.types) > 0:
            for pname, tpe in self.types.items():
                ...
                cg.line(typify(tpe, name))

            cg.line()

        cg.line(f"{tg_alias}.expose(")
        for name, fn in self.exposed.items():
            cg.line(f"{name}={typify(fn)},", 1)
        cg.line(")")
    ...
    def imp(self, gen: bool):
        ...
        wth.value = self.codegen(Codegen(), tg_alias=tg_alias).res
