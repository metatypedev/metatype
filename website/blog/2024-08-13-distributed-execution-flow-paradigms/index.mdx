...
Activities definition. Create a file for the workflow, e.g., orderWorkflow.ts:
...
Workflow definition. Create a file for the workflow, e.g., orderWorkflow.ts:
...
async function run() {
  ...
  const worker = await Worker.create({
    // for production, you want to bundle your workflows under a single file
    workflowsPath: require.resolve("orderWorkflow.ts"),
    activities,
...
typegraph(
  ...
  (g: any) => {
    ...
    g.expose(
      {
        start: temporal.startWorkflow("orderWorkflow", arg),
...
from .orderWorkflow import OrderWorkflow
...
def temporal(g: Graph):
  ...
  g.expose(
    public,
    start=temporal.start_workflow("orderWorkflow", arg),
