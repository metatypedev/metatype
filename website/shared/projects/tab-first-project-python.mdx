...
<CodeBlock language="python">
  {require("../../../examples/templates/python/api/example.py").content}
</CodeBlock>

Let's break down the above code snippet. The `example` function is where most of the our logic inside lives. We need to decorate the function with `@typegraph()` from the typegraph SDK. Inside the our function, we have different components of our application.

Metatype uses Policy Based [Access Control](/docs/reference/policies) for accessing resources in your backend and here we have defined a public access.
...
The `hello` endpoint is mapped with a `custom function` which accepts a `string` and returns **hello** concatinated with the `string`. The processing of data is done using the `PythonRuntime`. When we are defining the custom function, we pass the input type, output type and then a [function](/docs/concepts/mental-model#functions).
...
We are almost there to test your first `Metatype` application. We now need to spin a [Tyepgate](/docs/reference/typegate) and deploy our typegraph to the instance. We have two options to start a typegate instance. The first and the recommended one is to use the [embedded typgate](/docs/reference/meta-cli/embedded-typegate) that comes with the _non-thin_ version of _Meta_CLI_. The second option is to run a docker container from our `compose.yml` file.

To run the embedded typegate, you can follow this [reference](/docs/reference/meta-cli/embedded-typegate). You this option is preferrable as it spins a simple typegate for development.

To run the typegate container, run the following command.
```bash
...
Once you started your typegate instance using one of the available choice, if you open [localhost:7890](http://localhost:7890) on your browser, you will get a webpage similar to this one.
