...
Metatype supports multiple authentication schemes like Basic Auth, JSON Web Tokens (JWT) and OAuth2. This enables every request to have a context and store some information about the user. You can then use the context to set specific fields with `from_context` or as you will see next step, to restrict some accesses via the policies.
...
For OAuth2, the typegate provide some helpers to obtain the access token and renew them, either in the cookie - this works only when on the same subdomain due to third-party cookies restrictions - or in the `Authorization` header. Once setup, you can redirect your users to `http://localhost:7890/[typegraph]/auth/[auth name]` and the OAuth2 flow will be handled for you. The context will contain the user info of the provider. You can test it by setting up a [GitHub OAuth2 App](https://docs.github.com/en/developers/apps/building-oauth-apps/creating-an-oauth-app) or [try with the flow](https://demo.metatype.dev/authentication/auth/github?redirect_uri=https://metatype.dev/docs/tutorials/authentication-and-security) with the demo.
