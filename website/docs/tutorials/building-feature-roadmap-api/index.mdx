...
typegraph("roadmap", (g) => {
  ...
  const bucket = t.struct({
    ...
    "name": t.string(),
    "ideas": t.array(t.ref("idea")), // one-to many relationship
  ...
  const idea = t.struct({
    ...
    "name": t.string(),
    "authorEmail": t.email(),
    "bucket": t.ref("bucket"), // we need to specify the relationships on both types
...
`t.ref` declares logical relationships between our types which the Prisma runtime will be able to pick up.
...
We can use the `apply` method to modify the input types of functions.
...
Even though the `apply` method doesn't allow us to change the shape of the type, we can change the types of members and importantly here, hide the ones we don't need.
...
<TGExample
      typegraph="apply"
      python={require("./apply.py")}
      query={require("./apply.graphql")}
    />
...
It should be easy to expose a materializer for this using Prisma's `db.update` helper and `apply` to restrict changes to only those field.
