...
:::info Looking first for some explanation?
...
You will design some API blocks for a chat-based app where users can exchange messages and share products.

Running the ecosystem requires Python >= 3.8 which should be available on most the recent OS.
...
While the definition of typegraphs is currently done in Python, it doesn't require previous experience in Python.
The tutorial will cover all the basics and will not use any advanced features of the language.
:::
...
Finally download and launch a docker-compose manifest running a typegate node, a Redis instance (this is the sole dependency of the typegate) and a Postgres instance to store the app's messages.

```bash
curl -o docker-compose.yml \
  https://raw.githubusercontent.com/metatypedev/metatype/main/examples/docker-compose.yml
docker compose up -d
