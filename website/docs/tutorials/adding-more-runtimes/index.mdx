...
Getting random data is great, but having some input variables and meaningful output is even better. The random runtime is usually used to quickly draft and iterate on an API prototype. Once the interface feel efficient and well design, it's time to move another runtime backed by some storage.
...
Prisma is a "Next-generation Node.js and Typescript ORM" that support for PostgreSQL, MySQL/MariaDB, SQLite, MongoDB, CockroadDB and Microsoft SQL Server. It is one of the main runtimes provided by Metatype and doesn't require any additional installation.
...
1. Types get generated names unless manually specified them. You can find the exact names in the playground documentation. Here you want to have a human friendly name as it will also be the name of the table in your database.
...
2. The `from_parent` automatically fill the input type with the parent field named `uid`. The `g(·)` allows to make named reference to another type and avoid circular reference.
