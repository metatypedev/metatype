...
**This page will give you an introduction to the 80% of the Metatype concepts you will need a daily basis.**

:::note You will learn

- how to start your first typegraph
- how to model data with types
- how to manipulate data with runtimes
- how to secure data with policies
- how to deploy on a typegate node
...
Head over to the [mental model overview](/docs/concepts/mental-model) to understand the components.
:::
...
- quantifier types: `t.optional(·)`, `t.list(·)`, etc.
...
You can combine them with each other to describe almost any data type you may need. The typegate enforces the data validation when data flows through it. Some syntactic sugar is available to make the type definition shorter:
    ...
    <CodeBlock language="python">{require("../../../typegraphs/typecheck.py").content}</CodeBlock>
...
A complete typegraph definition may look like the following:
    ...
    <TGExample
          ...
          python={require("../../../typegraphs/first-typegraph.py")}
          typescript={require("!!code-loader!../../../typegraphs/first-typegraph.ts")}
