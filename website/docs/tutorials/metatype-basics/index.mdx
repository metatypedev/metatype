...
You can go ahead and try out the following `graphql` on the GraphiQL interface and get a feel for it.
```graphql
query {
  get_idea {
    id,
...
Meta-cli will auto-reload when it detects changes to `metatype.yaml` because Meta-cli was run in `dev` mode(through the `meta dev` command).
