...
Metatype also enables you to consume your API the same way you would consume regular REST APIs.

Setting up an http endpoint is as simple as calling a function: `g.rest(..)`.
...
* <b>query</b>: Perform a `GET` at `TYPEGATE_URL/TG_NAME/rest/endpoint_name?param1=..&param2=..`
* <b>mutation</b>: Perform a `POST` at `TYPEGATE_URL/TG_NAME/rest/endpoint_name` with `Content-Type` set as `application/json` 
```js
{
    "param1": ..,
    "param2": ..,
...
In the example above, for a local instance, the endpoint might look like:
`http://localhost:7890/example/rest/users?name=Bob&rows=10`
