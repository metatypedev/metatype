  ...
  [
    {
      ...
      body: (
        <>
          The typegate can be run as a single node mode for simple deployments and
          using the filesystem as it's artifact store. Useful for local
    ...
    }, 
    {
      ...
      body: (
        <>
          ...
          machines and scale horizontally. A Redis compatible KV deployment is
          needed for the message queue to coordiante the instances. An S3
          compatible object store is used for the artifacts. This mode is
          recommended for production deployments even if only a single node is
          used.
        </>
      ),
    }
...
They're built upon the typegraphs primitive that compose well with every other feature and allow [granular control](/docs/reference/types/parameter-transformations) when required.
