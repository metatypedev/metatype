...
Functions define how the input type gets transformed into the output type. The input and output types are similar to a function signature and the configuration + associated runtime to its implementation.

<CodeBlock language="python">{require("../../../../examples/typegraphs/functions.py").content}</CodeBlock>

## Runtimes

Every function have a runtime associated to it. This runtime describes where the input/output types are physically located. It can be another API, a database, or any other services the typegate can connect to. The typegates uses that information to optimize the execution of the queries and minimize the amount of data moved.
