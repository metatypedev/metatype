...
Every types and materializers have a runtime associated to it. This runtime describes where the type or materializer are physically located. It can be another API, a database, or any other services the typegate can connect to. The typegates uses that information to optimize the execution of the queries and minimize the amount of data moved.
