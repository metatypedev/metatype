...
:::warning Beta The following feature is currently only implemented for the `DenoRuntime`. :::
...
:::warning Beta The following feature is not yet stable. :::
...
Note, this also include typescript function types for specific typegraph functions. By default, the `mdk_typescript` generator will only include stub function types for those defined on the `DenoRuntime` but this is [configurable](/docs/reference/metagen#mdk_typescript).
