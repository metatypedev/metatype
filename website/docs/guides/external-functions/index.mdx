...
Note that for the `fib` root materializer, we're using an ECMA module in an external file.
...
- [`WasmedgeRuntime`](/docs/reference/runtimes/wasmedge/) through wasm modules.
- [`PrismaRuntime`](/docs/reference/runtimes/prisma/) throw raw SQL queries.

## Accessing funciton context
