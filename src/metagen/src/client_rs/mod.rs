...
fn render_client_rs(_config: &ClienRsGenConfig, tg: Arc<Typegraph>) -> anyhow::Result<String> {
    ...
    let manifest: ManifestPage<RsNodeMeta> = MetasPageBuilder::new(tg.clone())?.build()?;
    manifest.cache_references(&());
    let methods = manifest.render_all_buffered(&())?;
    with_metas_namespace(&mut buf, methods)?;
    let node_metas = manifest.get_cached_refs();

    with_types_namespace(&mut buf, types_rs)?;

    let name_memo: &IndexMap<TypeKey, String> = &name_memo;
    let page = selections::manifest_page(&tg)?;
    page.cache_references(&name_memo);
    page.render_all(&mut buf, name_memo)?;
    let selections = page.get_cached_refs();
