...
pub enum RpcCall {
    Reduceb { super_type_id: TypeId, entries: Vec<ReduceEntry> },
...
impl super::RpcDispatch for RpcCall {
    fn dispatch(self) -> Result<Value> {
        match self {
            ...
            Self::AddRawAuth { data } => Lib::add_raw_auth(data).map(|res| serde_json::to_value(res).unwrap()),
            ...
            Self::Oauth2WithExtendedProfiler { service_name, scopes, extension } => Lib::oauth2_with_extended_profiler(service_name, scopes, extension).map(|res| serde_json::to_value(res).unwrap()),
            ...
            Self::MetagenExec { config } => Lib::metagen_exec(config).map(|res| serde_json::to_value(res).unwrap()),
