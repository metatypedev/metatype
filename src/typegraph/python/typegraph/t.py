...
class typedef:
    ...
    def __repr__(self):
        return core.get_type_repr(self._id)
    ...
    def with_policy(self, *policies: Optional[PolicySpec]) -> Self:
        ...
        ret = copy.copy(self)
        ret._id = core.with_policy(self._id, new_policy_chain)
        ret.policy_chain = new_policy_chain
        return ret

    def rename(self, name: str) -> Self:
        res = core.rename_type(self._id, name)
        ret = copy.copy(self)
        ret._id = res
        ret.name = name
        return ret
    ...
    def _with_injection(self, injection: str) -> Self:
        res = core.with_injection(self._id, injection)
        ret = copy.copy(self)
        ret._id = res
...
def _with_ext(
    ...
    if as_id:
        type_id = core.as_id(type_id, as_id == "composite")
    config = serialize_config(raw_config)
    if config:
        type_id = core.with_config(type_id, config)
    if name:
        type_id = core.rename_type(type_id, name)
...
class integer(typedef):
    ...
    def __init__(
        ...
        *,
        ...
        res = core.integerb(data)
        super().__init__(_with_ext(res, as_id, config, name))
        self.min = min
        self.max = max
    ...
    def id(self, as_id: AsId = True) -> "typedef":  # "integer"
        res = core.as_id(self._id, as_id == "composite")
        return typedef(res)
    ...
    def __init__(
        ...
        *,
        ...
        data = TypeFloat(
            ...
            exclusive_minimum=(
                og_float(exclusive_minimum) if exclusive_minimum is not None else None
            ),
            exclusive_maximum=(
                og_float(exclusive_maximum) if exclusive_maximum is not None else None
            ),
            multiple_of=og_float(multiple_of) if multiple_of is not None else None,
            enumeration=enum,
        )
        res = core.floatb(data)
        super().__init__(_with_ext(res, None, config, name))
        self.min = min
        self.max = max
        self.exclusive_minimum = exclusive_minimum
        self.exclusive_maximum = exclusive_maximum
...
class func(typedef):
    ...
    def extend(self, props: Dict[str, typedef]):
        res = core.extend_struct(
            self.out._id, og_list((k, v._id) for k, v in props.items())
        )
    ...
    def reduce(self, value: Dict[str, Any]) -> "func":
        ...
        reduced_id = sdk_utils.reduceb(self._id, reduce_entries)
    ...
    def apply(self, value: ApplyParamObjectNode) -> "func":
        ...
        try:
            transform_data = core.get_transform_data(self.inp._id, transform_tree)
    ...
    def from_type_func(
        ...
        mat = Materializer(id=data.mat, effect="read")
        return func(
            typedef(id=data.inp),
            typedef(id=data.out),
            mat,
