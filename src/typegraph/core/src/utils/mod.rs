...
use common::typegraph::{Auth as CommonAuth, AuthProtocol};
use fs::FsContext;
...
use crate::{
    ...
    global_store::{get_sdk_version, Store},
    types::{builders, utils::Reduce},
    utils::metagen_utils::RawTgResolver,
};

use crate::types::{
    core::{TypeBase, TypeId as CoreTypeId, TypeStruct},
    utils::{Auth, FdkConfig, FdkOutput, QueryDeployParams},
    TypeId,
};
...
impl TryFrom<Oauth2Params<'_>> for String {
    ...
    fn try_from(value: Oauth2Params) -> Result<Self> {
        ...
        let ret = serde_json::to_string(&CommonAuth {
...
pub fn gen_reduceb(supertype_id: CoreTypeId, reduce: Reduce) -> Result<CoreTypeId> {
    ...
    while !item_list.is_empty() {
        ...
        if item.node.is_leaf() {
            ...
            if reduce_value.inherit && reduce_value.payload.is_none() {
                ...
                let new_id = builders::with_injection(id.into(), payload)?;
...
pub fn add_auth(data: Auth) -> Result<u32> {
    Store::add_auth(data)
}

pub fn add_raw_auth(data: String) -> Result<u32> {
    let raw_auth: CommonAuth = serde_json::from_str(&data).map_err(|e| e.to_string())?;
    Store::add_raw_auth(raw_auth)
}

pub fn oauth2(service_name: String, scopes: String) -> Result<String> {
    Oauth2Builder::new(scopes).build(named_provider(&service_name)?)
}

pub fn oauth2_without_profiler(service_name: String, scopes: String) -> Result<String> {
    Oauth2Builder::new(scopes)
        .no_profiler()
        .build(named_provider(&service_name)?)
}

pub fn oauth2_with_extended_profiler(
    service_name: String,
    scopes: String,
    extension: String,
) -> Result<String> {
...
pub fn oauth2_with_custom_profiler(
    service_name: String,
    scopes: String,
    profiler: CoreTypeId,
) -> Result<String> {
...
pub fn gql_deploy_query(params: QueryDeployParams) -> Result<String> {
    let query = r"
...
pub fn gql_remove_query(names: Vec<String>) -> Result<String> {
    let query = r"
...
pub fn metagen_exec(config: FdkConfig) -> Result<Vec<FdkOutput>> {
...
pub fn metagen_write_files(items: Vec<FdkOutput>, typegraph_dir: String) -> Result<()> {
