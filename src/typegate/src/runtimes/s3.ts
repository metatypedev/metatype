      ...
      switch (mat.name) {
        ...
        case "upload": {
          ...
          return stage.withResolver(async ({ path, file: f }) => {
            ...
            this.logger.info(
              `s3 upload: getting presigned URL for ${JSON.stringify(input)}`,
            );
            const command = new PutObjectCommand(input);
            ...
            if (!response.ok) {
              throw new Error(
                `Failed to upload file: ${response.statusText}: ${body}`,
              );
            }
        ...
        case "upload_all": {
          ...
          return stage.withResolver(async (args) => {
            ...
            await Promise.all(files.map(async (file, i) => {
              ...
              this.logger.info(
                `s3 upload ${i}: getting presigned URL for ${
              ...
              if (!response.ok) {
                throw new Error(
                  `Failed to upload file ${file.name}: ${response.statusText}: ${body}`,
                );
              }
