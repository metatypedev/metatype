...
Meta.test("deno(sdk): reduce", async (t) => {
  ...
  await t.should(
    ...
    async () => {
      await gql`
        query {
          testInvariant(student: { id: 1, name: "Jake", infos: { age: 15 } }) {
            student {
              id
              name
              infos {
  ...
  await t.should("work with injections", async () => {
    await gql`
      query {
        injectionInherit(student: { name: "Kyle" }) {
          student {
            id
            name
            infos {
...
Meta.test("python(sdk): reduce", async (t) => {
  const e = await t.engine("typecheck/reduce_py.py");
  await t.should(
    "work as normal if all nodes have g.inherit() flag",
    async () => {
      await gql`
        query {
          invariantReduce(
            one: "1"
            two: { reduce: 2, set: 3, user: 4, context: "5" }
          ) {
            one
            two {
              reduce
  ...
  await t.should("work with self-refering type", async () => {
    await gql`
      query {
        selfReferingType(
          a: "A1"
          b: { nested: { b: { nested: { a: "A3" } } } }
        ) {
          a # A1
          b {
            nested {
